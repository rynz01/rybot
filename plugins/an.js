...
anjaynolak = anjaynolak ? anjaynolak : {}
  if (m.chat in anjaynolak) throw 'Masih ada yang sedang membuat\nTeks Harta Tahta\ndi chat ini... tunggu sampai selesai'
  else anjaynolak[m.chat] = true
  m.reply('_Sedang membuat..._\n*Mohon tunggu sekitar 1 menit*')
  try {
    let img = await ht(text ? text : ':v')
    conn.sendFile(m.chat, img, 'Anjay Nolak.png', '*© Nurutomo*\nMade with FFmpeg', m)
  } finally {
    delete anjaynolak[m.chat]
  }
}
handler.help = ['anjay <teks>']
handler.tags = ['tools']
handler.command = /^((anjay)?nolak)$/i
handler.limit = true
...
let anjay = path.join(src, 'anjay')
function ht(text = '') {
  return new Promise((resolve, reject) => {
    let img = path.join(anjay, pickRandom(fs.readdirSync(anjay)))
    ...
    let layers = [
      ...
      textArgs('ANJAY', 'black', 'white', fsize, font, '(w-text_w)/2', `(h-text_h)/2-(text_h*${lh})`, w, h) + format + '[top]',
      textArgs('NOLAK', 'black', 'white', fsize, font, '(w-text_w)/2', `(h-text_h)/2`, w, h) + format + '[mid]',
    ...
    let o = 1 * new Date + '_anjay_nolak.png'
    o = path.join(tmp, o)
    let args = [
      '-y',
      '-i', img,
